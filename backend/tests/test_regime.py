...
def test_mean_reversion_fades_resistance_when_overbought():
    # Price tagging resistance (100.3) while overbought (RSI 80), mean below -> SHORT to the mean.
    ind = {"atr14": 1.0, "rsi14": 80.0, "last_close": 100.0, "ema20": 98.0}
...
def test_mean_reversion_fades_support_when_oversold():
    # Price tagging support (99.7) while oversold (RSI 20), mean above -> LONG to the mean.
    ind = {"atr14": 1.0, "rsi14": 20.0, "last_close": 100.0, "ema20": 102.0}
...
def test_mean_reversion_stands_aside_mid_range():
    ...
    ind = {"atr14": 1.0, "rsi14": 50.0, "last_close": 100.0, "ema20": 100.0}
...
def test_mean_reversion_skips_thin_reward():
    # At resistance + overbought, but the mean is barely below price -> reward too thin -> no trade.
    ind = {"atr14": 1.0, "rsi14": 80.0, "last_close": 100.0, "ema20": 99.95}
